            ...
            for file in zip_ref.namelist():
                ...
                if file.lower().endswith('fid') and f"{dir_path}/ACQUS".lower() in map(str.lower, zip_ref.namelist()):
                    ...
                    if len(path_parts) >= 3:
                        ...
                        # Check if extern_id has at least 8 characters
                        if len(extern_id) >= 8:
...
def search_in_xml(extern_id, target_position, year, fid_acqus_log, xml_match_log, match_process_log, error_log):
    ...
    try:
        ...
        with zipfile.ZipFile(xml_zip_path, 'r') as zip_ref:
            ...
            if year_folder_path not in zip_ref.namelist():
                error_log.error(f"Year folder '{year_folder_path}' does not exist in the ZIP.")
                return
...
# Function to process the MSP matches in the XML file
def process_msp_matches(analyte, file, fid_acqus_log, xml_match_log, match_process_log, error_log):
    ...
    try:
        ...
        # Check for discrepancies between the first and second match
        if genus_1 != genus_2 or (species_1 != species_2 and global_match_value_2 >= THRESHOLD):
            match_process_log.info(f"❌ extern_id = {extern_id} in {target_position} denied in {file}: Discrepancies in genus or species with high globalMatchValue ({global_match_value_2})")
            return

        # Process the third MSP match
        ...
        # Check for discrepancies between the first and third match
        if genus_1 != genus_3 or (species_1 != species_3 and global_match_value_3 >= THRESHOLD):
            match_process_log.info(f"❌ extern_id = {extern_id} in {target_position} denied in {file}: Discrepancies in genus or species with high globalMatchValue ({global_match_value_3})")
            return
